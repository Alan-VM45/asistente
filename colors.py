...
def capture():
    ...
    # Definición de los colores en rango HSV
    ...
    while True:
        comp, frame = cap.read()
        if comp:
            # Conversión del frame a HSV
            frame_HSV = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
            
            # Crear máscaras para el color amarillo y rojo
            ...
            draw(yellow_mask, [0, 255, 255], frame)  # Amarillo
            draw(red_mask, [0, 0, 255], frame)       # Rojo
    ...
    # Liberar la cámara y cerrar las ventanas después de salir del bucle
    cap.release()
